...
int solve(int x, int y, int l) {
    ...
    while (!q.empty()) {
        ...
        for (int dir = 0; dir < 4; dir++) {
            ...
            bool canMove = false;
            if (dir == 0 && left(cx, cy) && right(nx, ny)) canMove = true;  // left
            if (dir == 1 && right(cx, cy) && left(nx, ny)) canMove = true;  // right
            if (dir == 2 && up(cx, cy) && down(nx, ny)) canMove = true;     // up
            if (dir == 3 && down(cx, cy) && up(nx, ny)) canMove = true;     // down
